...
function buildPropertyCard(property) {
  ...
  const imageUrl = Array.isArray(property.imageUrls) && property.imageUrls.length > 0 ? property.imageUrls[0] : null;
  if (imageUrl) {
  ...
  location.textContent = `📍 ${property.location || "Unknown location"}`;
  ...
  if (property.description) {
    ...
    descChip.textContent = `${property.description.substring(0, 45)}${property.description.length > 45 ? "..." : ""}`;
  ...
  whatsappLink.href = whatsappNumber ? `https://wa.me/${whatsappNumber.replace(/[^0-9]/g, "")}` : "#";
