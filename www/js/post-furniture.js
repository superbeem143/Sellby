/* ===================================================== */
/*              SELLBY POST-FURNITURE.JS                 */
/*                     JS PART 1                         */
/* ===================================================== */
/*
    File Name : post-furniture.js
    Project   : SELLBY
...
async function uploadImages(furnitureId) {
    ...
    for (const file of selectedFiles) {
        ...
        const imageRef = ref(
            ...
            `furniture/${furnitureId}/${Date.now()}_${file.name}`
...
publishBtn.addEventListener("click", async () => {
    ...
    if (!title) {
        alert("Please enter the furniture title.");
        return;
    }
    if (!price) {
    ...
    statusMessage.textContent =
        "Uploading images and publishing your furniture ad...";
    /* ===================================================== */
/*              SELLBY POST-FURNITURE.JS                 */
/*                     JS PART 3                         */
/* ===================================================== */
...
/* ===================================================== */
    ...
    try {
        ...
        const newFurnitureRef = await addDoc(collection(db, "ads"), docData);

        const imageUrls = await uploadImages(newFurnitureRef.id);

        if (imageUrls.length) {
            await updateDoc(doc(db, "ads", newFurnitureRef.id), { imageUrls });
        }

        alert("Furniture Published Successfully!");
        ...
        alert("Failed to publish furniture. Please try again.");

        publishBtn.disabled = false;

        publishBtn.textContent = "Publish Furniture";
