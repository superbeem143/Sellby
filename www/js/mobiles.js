...
/* ===================================================== */
    ...
    const sellerId = mobile.userId || mobile.sellerId || '';
    ...
    whatsappLink.href =
        `chat.html?adId=${mobile.id}&sellerId=${sellerId}`;
...
async function loadMobiles() {
    try {
        ...
        const snapshot = await getDocs(mobilesQuery);
        ...
        snapshot.forEach((docSnapshot) => {
            ...
            mobileList.appendChild(buildMobileCard(data));
        });

    } catch (error) {
        console.error("Error loading mobiles:", error);
        loadingMessage.textContent = "Unable to load mobiles. Please try again later.";
    }
}
