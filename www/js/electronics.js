...
/* ===================================================== */
    ...
    whatsappLink.href =
        `chat.html?adId=${product.id}&sellerId=${sellerId}`;
...
async function loadElectronics() {
    try {
        ...
        const snapshot = await getDocs(electronicsQuery);
        ...
        snapshot.forEach((docSnapshot) => {
            ...
            electronicsList.appendChild(buildElectronicsCard(data));
        });

    } catch (error) {
        console.error("Error loading electronics:", error);
        loadingMessage.textContent = "Unable to load electronic products. Please try again later.";
    }
}
