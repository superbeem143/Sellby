...
publishBtn.addEventListener("click", async () => {
    ...
    if (!selectedFiles.length) {
        alert("Please add at least one photo of the car.");
        return;
    }
    if (!brand || !model) {
        alert("Please enter the car brand and model.");
        return;
    }
    if (!price) {
...
/* ===================================================== */
    ...
    try {
        ...
        const newCarRef = await addDoc(collection(db, "ads"), docData);

        const imageUrls = await uploadImages(newCarRef.id);

        if (imageUrls.length) {
            await updateDoc(doc(db, "ads", newCarRef.id), { imageUrls });
        }
